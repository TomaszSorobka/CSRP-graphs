    ...
    public void add(ModelContext ctx) throws GRBException {
        ...
        for (int i = 0; i < nStatements; i++) {
            for (int j = 0; j <= ctx.dimensions; j++) {
                ...
                s_y.addTerm(1.0, ctx.v.statementCoordinates[i][1]);
                ctx.model.addGenConstrIndicator(ctx.v.statementIsOnRow[i][j], 1, s_y, GRB.EQUAL, j, "row_match_" + j);
                ...
                ctx.model.addGenConstrIndicator(ctx.v.statementIsOnRow[i][j], 0, orExpr, GRB.GREATER_EQUAL, 1.0,
                        "is_onRow_j = 0 => s_y != " + j);
            }
        }
    }
