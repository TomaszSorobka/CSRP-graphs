    ...
    public void add(ModelContext ctx) throws GRBException {
        ...
        for (int i = 0; i < nEntities; i++) {
            if (ctx.inst.entityIndToStatements.get(ctx.entityIds.get(i)).length == 1) {
                ...
                // if row is active then the size of the row is 1 cell (for entities with 1 statement)
                for (int row = 0; row <= ctx.dimensions; row++) {
                    nrActiveRows.addTerm(1.0, ctx.v.entities[i].activeRows[row]);

                    GRBLinExpr sizeOfRow = new GRBLinExpr();
                    sizeOfRow.addTerm(1.0, ctx.v.entities[i].rowBounds[row][1]);
                    sizeOfRow.addTerm(-1.0, ctx.v.entities[i].rowBounds[row][0]);
                    ctx.model.addGenConstrIndicator(ctx.v.entities[i].activeRows[row], 1, sizeOfRow, GRB.EQUAL, 0, "single_cell_span");
                }
