    ...
    public void add(ModelContext ctx) throws GRBException {
        ...
        for (int i = 0; i < nStatements; i++) {
            for (int j = i + 1; j < nStatements; j++) {
                ...
                expr.addTerm(1.0, ctx.v.statementCoordinates[i][0]);
                expr.addTerm(-1.0, ctx.v.statementCoordinates[j][0]);
                ...
                expr = new GRBLinExpr();
                expr.addTerm(1.0, ctx.v.statementCoordinates[i][1]);
                expr.addTerm(-1.0, ctx.v.statementCoordinates[j][1]);
