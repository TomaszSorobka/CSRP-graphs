    ...
    public void add(ModelContext ctx) throws GRBException {
        ...
        for (int i = 0; i < nEntities; i++) {
            for (int j = 0; j < ctx.dimensions; j++) {
                ...
                leftOverlap.addTerm(1.0, ctx.v.entities[i].rowBounds[j][1]);
                leftOverlap.addTerm(-1.0, ctx.v.entities[i].rowBounds[j + 1][0]);

                ctx.model.addGenConstrIndicator(ctx.v.entities[i].activeRows[j], 1, leftOverlap,
                ...
                rightOverlap.addTerm(1.0, ctx.v.entities[i].rowBounds[j + 1][1]);
                rightOverlap.addTerm(-1.0, ctx.v.entities[i].rowBounds[j][0]);

                ctx.model.addGenConstrIndicator(ctx.v.entities[i].activeRows[j], 1, rightOverlap,
