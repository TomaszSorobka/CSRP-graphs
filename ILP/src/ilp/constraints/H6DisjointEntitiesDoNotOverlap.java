        ...
        for (int i = 0; i < nEntities; i++) {
            for (int j = i + 1; j < nEntities; j++) {
                ...
                if (!overlap) {
                    ...
                    expr.addTerm(-1.0, ctx.v.entityCoordinates[j][2]);
                    expr.addTerm(1.0, ctx.v.entityCoordinates[i][0]);
                    expr.addTerm(M, vars[0]);
                    ctx.model.addConstr(expr, GRB.GREATER_EQUAL, 0, "H6_" + i + "_" + j + "_left");

                    // x2_e1 - x1_e2 + M * vars[1] >= 0
                    expr = new GRBLinExpr();
                    expr.addTerm(1.0, ctx.v.entityCoordinates[j][0]);
                    ...
                    expr = new GRBLinExpr();
                    expr.addTerm(-1.0, ctx.v.entityCoordinates[j][3]);
                    expr.addTerm(1.0, ctx.v.entityCoordinates[i][1]);
                    expr.addTerm(M, vars[2]);
                    ctx.model.addConstr(expr, GRB.GREATER_EQUAL, 0, "H6_" + i + "_" + j + "_top");

                    // y1_e2 - y2_e1 + M * vars[3] >= 0
                    expr = new GRBLinExpr();
                    expr.addTerm(1.0, ctx.v.entityCoordinates[j][1]);
