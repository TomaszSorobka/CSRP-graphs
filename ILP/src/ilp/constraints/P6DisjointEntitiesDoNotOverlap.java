            ...
            for (int j = i + 1; j < nEntities; j++) {
                ...
                if (!overlap) {
                    for (int row = 0; row <= ctx.dimensions; row++) {
                        ...
                        exprForVarAbove.addTerm(1.0, ctx.v.entities[i].activeRows[row]);
                        exprForVarAbove.addTerm(1.0, ctx.v.entities[j].activeRows[row]);
                        ...
                        exprForVarAbove2.addTerm(1.0, ctx.v.entities[i].rowBounds[row][0]);
                        exprForVarAbove2.addTerm(-1.0, ctx.v.entities[j].rowBounds[row][1]);
                        ctx.model.addGenConstrIndicator(afterEntityJ, 1, exprForVarAbove2,
                                GRB.GREATER_EQUAL, 1.0, "e_" + i + "_row_" + j + "_afterEntityJ=1_implies_ei_after_ej");
                        ...
                        exprForVarAbove3.addTerm(1.0, ctx.v.entities[i].rowBounds[row][1]);
                        exprForVarAbove3.addTerm(-1.0, ctx.v.entities[j].rowBounds[row][0]);
                        ctx.model.addGenConstrIndicator(beforeEntityJ, 1, exprForVarAbove3,
                                GRB.LESS_EQUAL, -1.0, "e_" + i + "_row_" + j + "_beforeEntityJ=1_implies_ei_before_ej");
