...
public class PolygonAreaDimensionsComplexity implements ObjectiveModule {
    ...
    public void apply(ModelContext ctx) throws GRBException {
        ...
        objective.addTerm(1.0, ctx.v.maxWidth);
        objective.addTerm(1.0, ctx.v.maxHeight);
        ...
        for (int i = 0; i < nEntities; i++) {
            for (int j = 0; j <= ctx.dimensions; j++) {
                ...
                GRBVar span_j = ctx.model.addVar(0, ctx.dimensions + 1, 0, GRB.CONTINUOUS, "span_j");
                GRBLinExpr span_j_expr1 = new GRBLinExpr();
                span_j_expr1.addTerm(1.0, span_j);
                span_j_expr1.addTerm(-1.0, ctx.v.entities[i].rowBounds[j][1]);
                span_j_expr1.addTerm(1.0, ctx.v.entities[i].rowBounds[j][1]);
                ctx.model.addGenConstrIndicator(ctx.v.entities[i].activeRows[j], 1, span_j_expr1, GRB.EQUAL, 1.0,
                        "span_if_ej_1");

                // If e_j == 0 -> span_j = 0
                GRBLinExpr span_j_expr0 = new GRBLinExpr();
                span_j_expr0.addTerm(1.0, span_j);
                ctx.model.addGenConstrIndicator(ctx.v.entities[i].activeRows[j], 0, span_j_expr0, GRB.EQUAL, 0.0,
                        "span_if_ej_0");
    ...
    private static void addComplexity(ModelContext ctx, GRBLinExpr objective) throws GRBException {
        ...
        for (int i = 0; i < nEntities; i++) {
            for (int j = 0; j < ctx.dimensions; j++) {
                ...
                active_expr.addTerm(1.0, ctx.v.entities[i].activeRows[j]);
                active_expr.addTerm(1.0, ctx.v.entities[i].activeRows[j + 1]);
                ctx.model.addGenConstrIndicator(b_active_consecutive, 1, active_expr, GRB.EQUAL, 2,
                ...
                otherSide_expr.addTerm(1.0, ctx.v.entities[i].activeRows[j]);
                otherSide_expr.addTerm(1.0, ctx.v.entities[i].activeRows[j + 1]);
                ...
                GRBVar diff_InLeftSide = ctx.model.addVar(0.0, 1.0, 0.0, GRB.BINARY, "diff_in_left_size");

                GRBVar lesser = ctx.model.addVar(0.0, 1.0, 0.0, GRB.BINARY, "less_in_left_size");
                GRBVar larger = ctx.model.addVar(0.0, 1.0, 0.0, GRB.BINARY, "larger_in_left_size");

                GRBLinExpr difference = new GRBLinExpr();
                difference.addTerm(1.0, ctx.v.entities[i].rowBounds[j][0]);
                difference.addTerm(-1.0, ctx.v.entities[i].rowBounds[j + 1][0]);
                ...
                ctx.model.addGenConstrIndicator(diff_InLeftSide, 1, orExpr, GRB.GREATER_EQUAL, 1,
                ...
                GRBVar diff_InRightSide = ctx.model.addVar(0.0, 1.0, 0.0, GRB.BINARY, "diff_in_right_size");

                GRBVar lesser_right = ctx.model.addVar(0.0, 1.0, 0.0, GRB.BINARY, "less_in_right_size");
                GRBVar larger_right = ctx.model.addVar(0.0, 1.0, 0.0, GRB.BINARY, "larger_in_right_size");

                GRBLinExpr difference_right = new GRBLinExpr();
                difference_right.addTerm(1.0, ctx.v.entities[i].rowBounds[j][1]);
                difference_right.addTerm(-1.0, ctx.v.entities[i].rowBounds[j + 1][1]);
                ctx.model.addGenConstrIndicator(lesser_right, 1, difference_right, GRB.LESS_EQUAL, -1.0,
                        "rows_" + j + "_" + j + 1 + "_negative_difference_right_side");
                ctx.model.addGenConstrIndicator(larger_right, 1, difference_right, GRB.GREATER_EQUAL, 1.0,
                ...
                ctx.model.addGenConstrIndicator(diff_InRightSide, 1, orExpr_right, GRB.GREATER_EQUAL, 1,
                ...
                ctx.model.addConstr(larger_diff_right, GRB.GREATER_EQUAL, 0.0, "ran out of names3");
                ctx.model.addConstr(lesser_diff_right, GRB.GREATER_EQUAL, 0.0, "ran out of names4");

                // if b_active_consecutive (j, and j+1 are active), then sumComplexity =
                // diff_InLeftSide + diff_InRightSide
                GRBVar sumComplexity = ctx.model.addVar(0.0, 2.0, 0.0, GRB.INTEGER, "complexity_consecutive_rows_" + j);
                ...
                ctx.model.addGenConstrIndicator(b_active_consecutive, 1, expression, GRB.EQUAL, 0,
                ...
                ctx.model.addGenConstrIndicator(b_active_consecutive, 0, sumCompl_expr, GRB.EQUAL, 0,
                        "if_active_consecutive_rows_then_sumComplexity");
